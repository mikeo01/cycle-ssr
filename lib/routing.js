...
const deepFind = reduce((next, elm) =>
  isNil(next) ? undefined : next[elm] ?? next['/:id']
)
...
export const mapComponentToRoute = pipe(slugWithModule, fromPairs)
...
const tree = pipe(split('/'), filter(pipe(equals(''), not)), map(concat('/')))
...
export const withSlashes = map(compose(replace(/\/{2,}/, '/'), concat('/')))
export const splitSlug = pipe(
  split('/'),
  filter(
    pipe(anyPass([equals(''), equals('.'), equals('..'), equals('pages')]), not)
  )
)
