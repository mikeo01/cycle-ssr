...
program.name("cycle-ssr").version(pkg.version);
...
program
  ...
  .action(async () => {
    ...
    nodemon({ script: `${__dirname}/server.js`, watch: "dist/server" })
      ...
      .on("quit", () => {
        console.error("Application has terminated".red);
...
program
  ...
  .action(async () => {
    ...
    console.log(
      "All done. Use `npx cycle-ssr preview` to view your site in production mode"
        .green
    );
  });
program
  ...
  .action(async () => {
    ...
    exec(
      `node ${process.cwd()}/dist/server/cycle-ssg.cjs`,
      (error, stdout, stderr) => {
